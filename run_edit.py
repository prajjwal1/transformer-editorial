...
def preprocess_function(examples):
    prompt = str(examples[data_args.context])
    choice_0, choice_1, choice_2 = str(examples['thesis']), str(examples['anti-thesis']), str(examples['third-option'])
    choice_3 = str(examples['neg-thesis'])
    choices = [choice_0, choice_1, choice_2, choice_3]
    random.shuffle(choices)
    # Specify max_length in encoding when not using body
    encoding = tokenizer([prompt, prompt, prompt, prompt], choices, return_tensors='pt', max_length=data_args.max_length, padding='max_length', truncation=True)
